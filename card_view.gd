...
func generate_cards():
	...
	for card in card_list:
		...
		if(ran>=95):
			...
			texture = load(("res://Tank Cards/Rare/"+random_card+".png"))
			card.texture_normal = texture
			card.card_name = random_card
		elif(ran>=50):
			random_card = Global.uncommon_cards[randi_range(0,len(Global.uncommon_cards)-1)]
			texture = load("res://Tank Cards/Uncommon/"+random_card+".png")
			card.texture_normal = texture
			card.card_name = random_card
		else:
			random_card = Global.common_cards[randi_range(0,len(Global.common_cards)-1)]
			texture = load("res://Tank Cards/Common/"+random_card+".png")
			card.texture_normal = texture
			card.card_name = random_card
...
func _on_card_picked():
	...
	if(picks_left>0):
		...
		self.get_tree().change_scene_to_file("res://Stages/Rounds/round_scene.tscn") # THIS NEEDS TO BE EDITIED IN THE FUTURE FOR OTHER LEVELS. USE A GLOBAL FOR CURRENT LEVEL
